...
def extract_and_write_cookies(response, cookies_file, ignore_duplicates=False):
    """
    Extract cookies by parsing Set-Cookie headers manually. This avoids issues 
    caused by the internal cookie handling of requests or httpx.
    """

    # Attempt to get all Set-Cookie headers in a way compatible with both `requests` and `httpx`.
    # `httpx` supports `response.headers.get_list()`,
    # while `requests` can use `response.headers.get_all()` in newer versions.
    set_cookie_headers = []
    if hasattr(response.headers, "get_list"):
        ...
        # If neither get_list nor get_all is available, try a generic fallback:
        set_cookie = response.headers.get('Set-Cookie')
        if set_cookie:
            # If there's a single header, make it a list
            set_cookie_headers = [set_cookie]

    cookies = []
    for header in set_cookie_headers:
        # Each header usually looks like: 'name=value; Path=/; ...'
        # We'll just take the first 'name=value' part
    ...
    if ignore_duplicates:
        # If ignoring duplicates, keep only the last occurrence of each cookie
        cookie_dict = {}
        for name, value in cookies:
            cookie_dict[name] = value
        cookies = list(cookie_dict.items())
    # If not ignoring duplicates, include all occurrences as they appear

    # Convert to "name=value" format
    cookies_list = [f"{name}={value}" for name, value in cookies]
...
def main():
    ...
    is_http2 = request_data['http_version'].upper() == 'HTTP/2'
    
    # Create transport with SSL verification disabled for the proxy
    ...
    while True:
        try:
            ...
            if args.write_session_token:
                # LOCATION to modify the script as per your needs
                ...
                # LOCATION to modify the script as per your needs
                if extract_and_write_cookies(response, args.write_session_token, ignore_duplicates=True):
                    #print(f"Cookies are extracted and written to {args.write_session_token}")
                    if args.log_resp:
                        logging.info(f"\nCookies are extracted and written to {args.write_session_token}")
